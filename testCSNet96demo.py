...
from dwCSNet_model_demo import CSNet
...
#                 imgf_x = np.clip(img_recch[:row, :col]*255.0, 0, 255).astype(np.uint8)
                ...
                rec_PSNR = rec_PSNR + psnr_RefineGAN(imgf_x/255.0, img_orig[:,:,channel_no]/255.0)
...
#   To load the model, use the following code:
    ...
    print(output_data)
    output_psnr = "min= %.3f, Q1= %.3f, median= %.3f, Q3= %.3f, max= %.3f \n" % (np.min(PSNR_All), np.percentile(PSNR_All,25), np.median(PSNR_All), np.percentile(PSNR_All,75), np.max(PSNR_All))
    output_ssim = "min= %.3f, Q1= %.3f, median= %.3f, Q3= %.3f, max= %.3f \n" % (np.min(SSIM_All), np.percentile(SSIM_All,25), np.median(SSIM_All), np.percentile(SSIM_All,75), np.max(SSIM_All))
    output_nrmse = "min= %.3f, Q1= %.3f, median= %.3f, Q3= %.3f, max= %.3f \n" % (np.min(nrmse_All), np.percentile(nrmse_All,25), np.median(nrmse_All), np.percentile(nrmse_All,75), np.max(nrmse_All))
